...
#include "minco_smoother/trajectory.hpp"
...
/**
 * @class minco::BandedMatrix
 * @brief Banded matrix class for minco smoother
 */
class BandedMatrix
{
public:
...
public:
                                    ...
                                    Eigen::VectorXd& gradByTimes, double& gradByTailStateS)
  {
    ...
    for (int i = 0; i < N - 1; i++)
    {
      ...
      B1.row(4) = B1.row(3);
      // negative acceleration
      B1.row(5) = -(2.0 * b.row(i * 8 + 2) + 6.0 * T1(i) * b.row(i * 8 + 3) + 12.0 * T2(i) * b.row(i * 8 + 4) +
